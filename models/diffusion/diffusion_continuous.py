...
from models.util import sample_gaussian_like, get_mixed_prediction, trace_df_dx_hutchinson
    ...
    def compute_ode_nll(self, dae, eps, ode_eps, ode_solver_tol, enable_autocast, no_autograd, num_samples, report_std):
        ...
        for i in range(num_samples):
            ...
            if self.sde_type == 'vesde':
                logp_prior = torch.sum(log_p_var_normal(x_t0, var=self.sigma2_max), dim=[1, 2, 3])
            else:
                logp_prior = torch.sum(log_p_standard_normal(x_t0), dim=[1, 2, 3])
