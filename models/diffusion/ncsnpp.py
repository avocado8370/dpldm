...
class NCSNpp(nn.Module):
    ...
    def __init__(self, args, num_input_channels, ch_mult, diffusion):
        super().__init__()
    ...
    def forward(self, x, t):
        ...
        return h
    def train_losses(self, x_start, args):
        noise = torch.randn_like(x_start)

        # apply diffusion model for samples generated for q (vae)
        t_p, var_t_p, m_t_p, obj_weight_t_p, _, g2_t_p = \
            self.diffusion.iw_quantities(args.batch_size, args.time_eps, args.iw_sample_q, args.iw_subvp_like_vp_sde)
        eps_t_q = self.diffusion.sample_q(x_start, noise, var_t_p, m_t_p)

        mixing_component = self.diffusion.mixing_component(eps_t_q, var_t_p, t_p, True)
        pred_params_q = self.forward(eps_t_q, t_p)
        params = get_mixed_prediction(True, pred_params_q, self.mixing_logit, mixing_component)
        l2_term_q = torch.square(params - noise)

        p_objective = torch.sum(obj_weight_t_p * l2_term_q, dim=[1, 2, 3])
    ...
    def get_optimizer(self, args):
        dae_optimizer = FusedAdam(self.parameters(), args.learning_rate, weight_decay=args.weight_decay, eps=1e-4)
        dae_optimizer = EMA(dae_optimizer, ema_decay=args.ema_decay)
        return dae_optimizer
